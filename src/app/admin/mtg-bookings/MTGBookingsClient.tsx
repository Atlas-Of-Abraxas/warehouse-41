...
export default function MTGBookingsClient({ initial }: { initial: MTGBookingRow[] }) {
          ...
          <table className="w-full text-sm min-w-[1100px]">
            <thead>
              <tr className="border-b border-[var(--color-border)]">
                ...
                <th className="text-left p-3 text-[var(--color-text-secondary)]">Table</th>
                <th className="text-left p-3 text-[var(--color-text-secondary)]">Format</th>
                <th className="text-left p-3 text-[var(--color-text-secondary)]">After hours</th>
            ...
            <tbody>
              {filtered.map((b) => (
                <MTGRow key={b.id} row={b} onSaved={() => router.refresh()} />
              ))}
            </tbody>
          </table>
        </div>
...
function MTGRow({
  ...
  return (
    <tr className="border-b border-[var(--color-border)] last:border-0 align-top">
      ...
      <td className="p-3 text-xs">{row.table ?? "—"}</td>
