...
export default function KillTeamBookingsClient({
  initial,
}: {
  initial: KTBookingRow[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return initial.filter((b) => {
      ...
      return (
        ...
        b.terrain.toLowerCase().includes(q) ||
        b.teamsLabel.toLowerCase().includes(q)
      );
    });
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <label className="text-sm">
          ...
          <input
            ...
            placeholder="Name, email, terrain…"
          ...
          <table className="w-full text-sm min-w-[960px]">
            <thead>
              <tr className="border-b border-[var(--color-border)]">
                ...
                <th className="text-left p-3 text-[var(--color-text-secondary)]">Terrain</th>
                <th className="text-left p-3 text-[var(--color-text-secondary)]">Teams</th>
            ...
            <tbody>
              {filtered.map((b) => (
                <KTRow key={b.id} row={b} onSaved={() => router.refresh()} />
              ))}
            </tbody>
          </table>
        </div>
...
function KTRow({
  ...
  return (
    <tr className="border-b border-[var(--color-border)] last:border-0 align-top">
      ...
      <td className="p-3">{row.terrain}</td>
      <td className="p-3 max-w-[200px] text-xs">{row.teamsLabel}</td>
      <td className="p-3">
